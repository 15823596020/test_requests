...
class Label(WeWork): # 继承WeWork类
    def __init__(self):
        self.data = self.yaml_load("../data/config.yaml")

    #  创建标签，create_label_tagid方法，需传入参数tagname, tagid
    def create_label_tagid(self, tagname, tagid):
        req = {
            "method": self.data["create_label"]["method"],  #"post",
            "url": self.data["create_label"]["url"],  #"https://qyapi.weixin.qq.com/cgi-bin/tag/create",
    ...
    def update_label(self, tagid, tagname):
        req = {
            "method": self.data["update_label"]["method"],  #"post",
            "url": self.data["update_label"]["url"],  #"https://qyapi.weixin.qq.com/cgi-bin/tag/update",
            "params": {"access_token": self.token},
    ...
    def delete_label(self, tagid):
        req = {
            "method": self.data["delete_label"]["method"],  #"get",
            "url": self.data["delete_label"]["url"],  #"https://qyapi.weixin.qq.com/cgi-bin/tag/delete",
    ...
    def get_labelmember(self, tagid):
        req = {
            "method": self.data["get_labelmember"]["method"],  #"get",
            "url": self.data["get_labelmember"]["url"],  #"https://qyapi.weixin.qq.com/cgi-bin/tag/get",
            "params": {"access_token": self.token, "tagid": tagid}
        }
        r = self.send_api(req)  # 调用send_api方法，并传入一个json结构体的请求信息
        print(r.json())
    ...
    def create_labelmember_all(self,tagid,userlist,partylist):
        req = {
            "method": self.data["create_labelmember"]["method"],  #"post",
            "url": self.data["create_labelmember"]["url"],  #"https://qyapi.weixin.qq.com/cgi-bin/tag/addtagusers",
            "params": {"access_token": self.token},
    ...
    def delete_labelmember_all(self,tagid,userlist,partylist):
        req = {
            "method": self.data["delete_labelmember"]["method"],  #"post",
            "url": self.data["delete_labelmember"]["url"],  #"https://qyapi.weixin.qq.com/cgi-bin/tag/deltagusers",
            "params": {"access_token": self.token},
            "json": {"tagid": tagid, "userlist": userlist, "partylist": partylist}
        }
        r = self.send_api(req)  # 调用send_api方法，并传入一个json结构体的请求信息
    ...
    def get_labellist(self):
        req = {
            "method": self.data["get_labellist"]["method"],  #"get",
            "url": self.data["get_labellist"]["url"],  #"https://qyapi.weixin.qq.com/cgi-bin/tag/list",
            "params": {"access_token": self.token},
        }
        r = self.send_api(req)  # 调用send_api方法，并传入一个json结构体的请求信息
        print(r.json())
